...
export function useAdminNotifications() {
  ...
  const [readIds, setReadIds] = useState(() => {
    try {
      const saved = localStorage.getItem('admin_read_ids');
...
const fetchPending = useCallback(async () => {
  try {
    const res = await api.get('/admin/peminjaman/pending-count');
    const newPending = res.data.data ?? [];
    setPendingCount(res.data.pending_count ?? 0);
    setPending(newPending);

    // ← tambah ini: bersihkan readIds yang ID-nya sudah tidak ada di pending
    const activeIds = newPending.map(p => Number(p.id));
    setReadIds(prev => {
      const cleaned = prev.filter(id => activeIds.includes(Number(id)));
      localStorage.setItem('admin_read_ids', JSON.stringify(cleaned));
      return cleaned;
    });
  ...
  const markAllRead = () => {
    ...
    localStorage.setItem('admin_read_ids', JSON.stringify(merged));
    setReadIds(merged);
  };
  ...
  const deleteAllRead = () => {
    ...
    localStorage.setItem('admin_read_ids', JSON.stringify(cleanedReadIds));
