...
import { LayoutDashboard, Package, ArrowLeftRight, LogOut, Menu, X, Bell, CheckCheck } from 'lucide-react';
import { Link, useLocation, Outlet, useNavigate } from 'react-router-dom';
import api from '../services/api';
import { usePetugasNotifications } from '../hooks/usePetugasNotifications';
...
export default function PetugasLayout() {
  ...
  const {
    ...
    formatTime, readIds,
  } = usePetugasNotifications();

  useEffect(() => {
    const token = localStorage.getItem('token');
    const role = localStorage.getItem('role');
    if (!token || role !== 'petugas') navigate('/login', { replace: true });
  }, [navigate]);
            ...
            <div ref={bellRef} className="relative">
              ...
              {open && (
                <div style={{
                  ...
                  {pendingCount > 5 && (
                    <div
                      onClick={() => { setOpen(false); navigate('/petugas/peminjaman'); }}
                      style={{
