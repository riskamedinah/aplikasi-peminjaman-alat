      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl mx-4 flex flex-col max-h-[92vh]">
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-4">
          ...
          <div>
            ...
            <input type="date" className={inp('tanggal_pinjam')} value={form.tanggal_pinjam}
              onChange={e => set('tanggal_pinjam', e.target.value)} />
          ...
          <div>
            ...
            <input type="date" className={inp('tanggal_kembali_rencana')} value={form.tanggal_kembali_rencana}
              onChange={e => set('tanggal_kembali_rencana', e.target.value)} />
          ...
          <div>
            ...
            {errors.status && <p className="text-xs text-red-500 mt-1">{errors.status[0]}</p>}
          </div>
          <div>
            <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1.5">Keperluan</label>
            <textarea rows={3} className={inp('keperluan')} value={form.keperluan}
...
const AddPeminjamanModal = ({ users, alats, currentUser, onClose, onSaved }) => {
  ...
  const handleSelectPeminjam = (user) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (form.keperluan.trim().length < 3) errs.keperluan = ['Keperluan minimal 3 karakter'];
    ...
    setErrors({});
    try {
      await api.post('/admin/peminjaman', {
        ...form,
  ...
  return (
    <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/50 backdrop-blur-sm">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl mx-4 flex flex-col max-h-[90vh]">
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5">
          ...
          <div className="grid grid-cols-1 gap-4 mb-6">
            ...
            <div ref={dropdownRef} className="relative">
              ...
              <button
                type="button"
                onClick={() => setPeminjamDropdownOpen(o => !o)}
                  ...
                  <div className="px-3 py-3">
                    <div className="relative">
                      ...
                      <input
                        ...
                        className="w-full pl-10 pr-3 py-2 text-sm rounded-xl border border-gray-200 focus:border-[#3F51B5] focus:ring-2 focus:ring-[#3F51B5]/10 outline-none"
                      />
                    </div>
                  </div>
                  <div className="max-h-64 overflow-y-auto divide-y divide-gray-100">
                    {peminjamanList.length === 0 ? (
                      ...
                      <button
                        key={u.id}
                        type="button"
                        onClick={() => handleSelectPeminjam(u)}
          ...
          <div className="mb-6">
            <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1.5">Keperluan *</label>
            <textarea rows={2} className={inp('keperluan')} value={form.keperluan} onChange={e => set('keperluan', e.target.value)} placeholder="Sebutkan keperluan peminjaman..." />
            {errors.keperluan && <p className="text-xs text-red-500 mt-1">{errors.keperluan[0]}</p>}
          </div>
          ...
          <div className="mb-6">
            ...
            <div className="bg-gray-50 rounded-xl p-4 mb-3 border border-gray-200">
              <div className="grid grid-cols-3 gap-2 mb-2">
                <select value={selectedAlat} onChange={e => setSelectedAlat(e.target.value)} style={{ background: 'white' }} className="px-3 py-2 text-sm rounded-xl border border-gray-200 outline-none focus:border-[#3F51B5] focus:ring-2 focus:ring-[#3F51B5]/10">
                  <option value="">Pilih Alat</option>
                  {alats.map(a => (
                ...
                {details.map((d, idx) => (
                  <div key={idx} className="flex items-center justify-between bg-gray-50 rounded-xl px-4 py-3 border border-gray-200">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <button type="button" onClick={() => removeDetail(idx)} className="px-2 py-1 text-xs font-medium text-red-600 hover:bg-red-50 rounded-lg transition-colors">
                        ✕ Hapus
                      </button>
                    </div>
                  </div>
        ...
        {/* Footer */}
        <div className="flex gap-3 px-6 py-4 border-t border-gray-100">
          <button type="button" onClick={onClose} disabled={loading} className="flex-1 py-2.5 rounded-xl border border-gray-200 text-sm font-medium text-gray-600 hover:bg-gray-50 transition-colors disabled:opacity-50">
            Batal
          </button>
          <button onClick={handleSubmit} disabled={loading} className="flex-1 py-2.5 rounded-xl text-sm font-semibold text-white transition-all disabled:opacity-50 flex items-center justify-center gap-2" style={{ background: '#3F51B5' }}>
            {loading && <Spinner />}
            Tambah Peminjaman
          </button>
        </div>
...
function Peminjaman() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const res = await api.get('/admin/peminjaman', { params });
  ...
  useEffect(() => {
    setPage(1);
  }, [search, filterStatus, tanggal, sortBy, sortOrder]);
  ...
  const handleExport = async (format) => {
    ...
    try {
      ...
      const res = await api.get('/admin/peminjaman/export', { params, responseType: 'blob' });
      ...
      a.href = url;
      a.download = `laporan-peminjaman-${new Date().getTime()}.${ext}`;
      a.click();
  ...
  const handleDelete = async () => {
    ...
    try {
      await api.delete(`/admin/peminjaman/${deleteTarget.id}`);
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6">
      ...
      <div className="mb-6">
        ...
        <p className="text-sm text-gray-500 mt-0.5">Monitor dan kelola seluruh transaksi peminjaman alat</p>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 mb-4">
        ...
        <div className="flex flex-wrap items-center gap-3 mt-3">
          <div className="flex items-center gap-1.5 text-xs text-gray-400">
            <CalendarIcon />
            <span>Filter tanggal pinjam:</span>
          </div>
          <input
            type="date"
            value={tanggal}
        ...
        {hasFilter && (
          <div className="flex items-center gap-2 mt-3 flex-wrap">
            ...
            {tanggal && (
              <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium bg-[#3F51B5]/10 text-[#3F51B5]">
                {fmt(tanggal)}
                <button onClick={() => setTanggal('')} className="hover:opacity-70 ml-0.5">×</button>
              </span>
            )}
                      ...
                      <tr key={row.id} className="hover:bg-gray-50/50 transition-colors group">
                        ...
                        <td className="px-4 py-3.5">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <p className="font-medium text-gray-800 text-sm leading-tight">{row.user?.name}</p>
                        ...
                        <td className="px-4 py-3.5">
                          <div className="flex flex-wrap gap-1">
                            {row.details?.slice(0, 2).map(d => (
                              <span key={d.id} className="inline-block text-xs px-2 py-0.5 rounded-lg bg-gray-100 text-gray-600 font-medium">
                                {d.alat?.nama_alat} ×{d.jumlah}
                              </span>
                            ))}
                            {(row.details?.length ?? 0) > 2 && (
                              <span className="inline-block text-xs px-2 py-0.5 rounded-lg bg-gray-100 text-gray-400">
